...
int err_msg_ret(const int err_code, const std::string &err_info, const std::string &file_name,
    ...
    if (*out_size < (int)(output_str.length() + 1)) {
        return -3;
    }
...
bool serialize_json_node(const nlohmann::json &json, char *out, int *out_size, std::string &err_msg) {
    std::string out_str = json.dump();

    if (!out || !out_size) {
        err_msg = "Invalid pointer (out/out_size).";
        return false;
    }

    if (*out_size < (int)(out_str.length() + 1)) {
